...
export class AdvisorAgentService {
   ...
   */
  async getLatestSummary(userId: string): Promise<AdvisorSummary | null> {
    const result = await query(
      `SELECT * FROM advisor_summaries 
       WHERE user_id = $1 
  ...
  private async getAgentActivity(
    ...
    const result = await query(
      `SELECT 
       ...
       WHERE a.user_id = $1
       GROUP BY a.id, a.name, a.role_type`,
      [userId, start, end]
    );
   ...
   */
  private async getConversations(userId: string, start: Date, end: Date): Promise<any[]> {
    const result = await query(
   ...
   */
  private async getBusinessEvents(userId: string, start: Date, end: Date): Promise<any[]> {
    const result = await query(
  ...
  private async generateInsights(
    ...
    // Most active agent
    const mostActive = agentActivity.reduce((prev, current) =>
    ...
    if (events.length > 0) {
      ...
      const topEvent = Object.entries(eventTypes).sort((a: any, b: any) => b[1] - a[1])[0];
      insights.push({
        category: 'customer',
   ...
   */
  private async identifyConflicts(conversations: any[]): Promise<any[]> {
   ...
   * Generate forecasts
   */
  private async generateForecasts(agentActivity: any[], events: any[]): Promise<any[]> {
    const forecasts: any[] = [];
...
export default new AdvisorAgentService();
